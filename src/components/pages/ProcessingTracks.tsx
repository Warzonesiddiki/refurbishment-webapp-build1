...
export function ProcessingTracks() {
  ...
  const trackCounts = useMemo(() => {
    ...
    Object.keys(trackConfig).forEach((track) => counts.set(track, new Set<string>()));

    state.laptops.forEach((laptop) => {
      const track = normalizeTrackLabel(laptop.track);
      const bucket = counts.get(track);
      if (!bucket) return;
      bucket.add(normalizeBarcode(laptop.barcode));
    });

    state.wipJobs.forEach((job) => {
      if (job.status === "Completed") return;
      const track = normalizeTrackLabel(job.track);
      const bucket = counts.get(track);
      if (!bucket) return;
      bucket.add(normalizeBarcode(job.laptop));
    });

    return Object.fromEntries(Array.from(counts.entries()).map(([track, barcodes]) => [track, barcodes.size]));
  }, [state.laptops, state.wipJobs]);

  const trackLaptops = useMemo(() => {
    type TrackLaptopCard = {
      id: string;
      barcode: string;
      brand: string;
      model: string;
    ...
    const cards = new Map<string, TrackLaptopCard>();

    state.laptops.forEach((laptop) => {
      if (normalizeTrackLabel(laptop.track) !== activeTrack) return;
      const normalizedBarcode = normalizeBarcode(laptop.barcode);
      cards.set(normalizedBarcode, {
        id: laptop.id,
    ...
    state.wipJobs.forEach((wip) => {
      if (normalizeTrackLabel(wip.track) !== activeTrack || wip.status === "Completed") return;
      const normalizedBarcode = normalizeBarcode(wip.laptop);
      if (cards.has(normalizedBarcode)) return;

      const brandTokens = wip.brand.trim().split(/\s+/).filter(Boolean);
      const inferredBrand = brandTokens[0] || "Unknown";
      const inferredModel = brandTokens.slice(1).join(" ") || "Model Unknown";

      cards.set(normalizedBarcode, {
        id: `wip-${normalizedBarcode}`,
        barcode: wip.laptop,
        brand: inferredBrand,
        model: inferredModel,
        status: wip.status,
    ...
    return Array.from(cards.values()).filter((laptop) => {
      if (!normalizedSearch) return true;
      return [laptop.barcode, laptop.brand, laptop.model, laptop.status].join(" ").toLowerCase().includes(normalizedSearch);
    });
  ...
  const moveLaptop = (barcode: string, toStage: string) => {
    const normalizedBarcode = normalizeBarcode(barcode);
    const wip = getActiveTrackWip(normalizedBarcode);

    if (!wip) {
      trigger("warn", `No active WIP found for ${normalizedBarcode}. Create/activate WIP first.`);
      return;
    }
    ...
    logMove(normalizedBarcode, { from: currentStage, to: toStage });
    dispatch({
      type: "UPDATE_WIP",
      id: wip.id,
      payload: {
    ...
    dispatch({
      type: "ADD_ACTIVITY",
      payload: { action: `${normalizedBarcode} moved to ${toStage} on ${activeTrack}`, time: "just now" },
    });
    trigger("success", `${normalizedBarcode} → ${toStage}`);
    setDragItem(null);
  };
                ...
                {stageLaptops.length === 0 ? (
                  ...
                  stageLaptops.map((laptop) => (
                    <div
                      key={laptop.id}
                      draggable
                      onDragStart={() => setDragItem(laptop.barcode)}
                      onDragEnd={() => setDragItem(null)}
                      className={`p-2.5 rounded-lg bg-cyan-500/5 border border-cyan-500/10 cursor-grab hover:border-cyan-500/25 transition-all ${dragItem === laptop.barcode ? "opacity-50 scale-95" : ""}`}
                    >
                      <p
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        ...
                        {laptop.source === "laptop" ? (
                          ...
                          <span className="text-[9px] text-cyan-500/25">WIP-only</span>
                        )}
                      </div>
                    </div>
                  ))
