...
export function InventoryParts() {
  ...
  const [newPart, setNewPart] = useState<AddPartPayload>({
    barcode: "",
    name: "",
    category: "",
    spec: "",
  ...
  const addPart = () => {
    const normalizedBarcode = newPart.barcode.trim().toUpperCase();
    const normalizedName = newPart.name.trim();
    if (!normalizedBarcode || !normalizedName) {
      trigger("error", "Barcode and name are required");
      return;
    }

    if (state.parts.some((part) => part.barcode.trim().toUpperCase() === normalizedBarcode)) {
      trigger("error", `Part ${normalizedBarcode} already exists`);
      return;
    }
    ...
    dispatch({
      ...
      payload: {
        ...
        name: normalizedName,
        onHand: Math.max(0, newPart.onHand),
        available: Math.max(0, newPart.onHand),
        reorder: Math.max(0, newPart.reorder),
        cost: Math.max(0, newPart.cost),
      },
    });
    dispatch({ type: "ADD_ACTIVITY", payload: { action: `Added part ${normalizedBarcode}`, time: "just now" } });
    setNewPart({
      barcode: "",
      name: "",
      category: "",
  ...
  return (
    <div data-page="inventory-parts" data-testid="page-inventory-parts" className="space-y-6">
              ...
              lines.slice(1).forEach((line) => {
                ...
                dispatch({
                  ...
                  payload: {
                    ...
                    available: Number(row.Available || row.OnHand || 0),
                    reorder: Number(row.Reorder || 5),
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <KpiCard
          label="In Stock"
          value={state.parts.filter((p) => p.onHand > p.reorder).length}
          tone="green"
          icon="✓"
        />
        ...
        <div className="glass-card neon-border p-5 space-y-4 animate-slide-up">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <input
              ...
              onChange={(e) =>
                setNewPart((p) => ({ ...p, onHand: Number(e.target.value), available: Number(e.target.value) }))
              }
              className="px-3 py-2 rounded-lg text-sm"
            />
            <input
              ...
              value={newPart.reorder || ""}
              onChange={(e) => setNewPart((p) => ({ ...p, reorder: Number(e.target.value) }))}
              className="px-3 py-2 rounded-lg text-sm"
            />
            <input
              type="number"
              ...
              value={newPart.cost || ""}
              onChange={(e) => setNewPart((p) => ({ ...p, cost: Number(e.target.value) }))}
              className="px-3 py-2 rounded-lg text-sm"
              style={{ fontFamily: "var(--font-mono)" }}
            />
            <input
              ...
              className="px-3 py-2 rounded-lg text-sm"
            />
            <button className="btn-cyber" onClick={addPart}>
              ✓ Add Part
            </button>
          </div>
        </div>
        ...
        <div className="glass-card neon-border p-5 space-y-4 animate-slide-up">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <input
              placeholder="Specification"
              value={editData.spec || ""}
              onChange={(e) => setEditData((p) => ({ ...p, spec: e.target.value }))}
              className="px-3 py-2 rounded-lg text-sm"
            />
            <input
              type="number"
              ...
              onChange={(e) => setEditData((p) => ({ ...p, onHand: Number(e.target.value) }))}
              className="px-3 py-2 rounded-lg text-sm"
            />
            <input
              type="number"
              ...
              onChange={(e) => setEditData((p) => ({ ...p, reorder: Number(e.target.value) }))}
              className="px-3 py-2 rounded-lg text-sm"
            />
            <input
              type="number"
            ...
            <button
              ...
              onClick={() => {
                ...
                dispatch({
                  type: "ADD_ACTIVITY",
                  payload: {
                    action: `Stock ${adjustType}: ${showAdjust.barcode} ${delta} (${adjustReason || "no reason"})`,
                    time: "just now",
                  },
                });
                setShowAdjust(null);
      ...
      {historyPart && (
        <div className="glass-card neon-border p-5 space-y-4 animate-slide-up">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold neon-text-purple" style={{ fontFamily: "var(--font-heading)" }}>
              PART HISTORY — {historyPart.barcode}
            </h3>
            <button className="btn-ghost text-xs" onClick={() => setHistoryPart(null)}>
              ✕ Close
            </button>
          </div>
          <div className="space-y-2">
            {state.activity.slice(0, 6).map((a, i) => (
              <div key={i} className="text-xs text-cyan-100/60">
                • {a.action} <span className="text-cyan-500/30">({a.time})</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search barcode, name, category..."
      ...
      <div className="glass-card corner-marks p-0 overflow-hidden">
                  ...
                  return (
                    <tr key={row.id}>
                      ...
                      <td className="py-3 px-4">
                        <span className="cyber-chip">{row.condition}</span>
                      </td>
                      <td
                        className={`py-3 px-4 text-right font-bold ${stockStatus === "out" ? "text-red-400" : stockStatus === "low" ? "text-yellow-400" : "neon-text-green"}`}
                        style={{ fontFamily: "var(--font-mono)" }}
                      >
                        {row.onHand}
                      </td>
                      <td className="py-3 px-4 text-right text-cyan-300/50" style={{ fontFamily: "var(--font-mono)" }}>
                        {row.available}
                      </td>
                      <td className="py-3 px-4 text-right text-cyan-500/30" style={{ fontFamily: "var(--font-mono)" }}>
                        {row.reorder}
                      </td>
                      <td
                        className="py-3 px-4 text-right font-semibold neon-text-green"
                        style={{ fontFamily: "var(--font-mono)", fontSize: "12px" }}
                      >
                        AED {row.cost}
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex gap-2">
                          ...
                          <button
                            className="text-[11px] text-cyan-400/50 hover:text-cyan-300 transition-colors font-semibold"
                            onClick={() => setShowAdjust(row)}
                          >
                            Adjust
                          </button>
                          <button
                            className="text-[11px] text-cyan-400/50 hover:text-cyan-300 transition-colors font-semibold"
                            onClick={() => setHistoryPart(row)}
                          >
                            History
                          </button>
                        </div>
        ...
        <div className="px-4 py-3 border-t border-cyan-500/10 flex items-center justify-between">
          <span className="text-[11px] text-cyan-500/30" style={{ fontFamily: "var(--font-mono)" }}>
            Showing {Math.min((page - 1) * pageSize + 1, filtered.length)}–{Math.min(page * pageSize, filtered.length)}{" "}
            of {filtered.length}
          </span>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p) => (
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded text-[11px] font-bold transition-all ${p === page ? "bg-cyan-500/15 text-cyan-300 border border-cyan-500/30" : "text-cyan-500/20 hover:text-cyan-400 border border-transparent"}`}
                  style={{ fontFamily: "var(--font-heading)" }}
                >
                  {p}
                </button>
