...
export function ReceivingVerification() {
  ...
  useEffect(() => {
    ...
    if (selectedLot && state.lots.some((lot) => lot.id === selectedLot.id)) return;

    const pendingLot = state.lots.find((lot) => lot.status !== "Verified") ?? state.lots[0];
    setSelectedLot(pendingLot ?? null);
  }, [selectedLot, state.lots]);
  ...
  const pendingCount = lotLaptops.filter((l) => l.status === "Pending Verification").length;
  ...
  const handleVerify = () => {
    ...
    if (!q) return;
    const laptop = state.laptops.find((item) => item.barcode.toUpperCase() === q);
    if (!laptop || (selectedLot && laptop.lot !== selectedLot.lot)) {
      trigger("error", "Scan mismatch: not in selected lot");
      return;
    }

    if (laptop.status !== "Pending Verification") {
      trigger("warn", `${laptop.barcode} is already verified`);
      setScanValue("");
      return;
    }
  ...
  const filteredRows = useMemo(() => {
    const tabRows =
      tab === "all"
        ? lotLaptops
        : tab === "verified"
          ? lotLaptops.filter((l) => l.status !== "Pending Verification")
          : lotLaptops.filter((l) => l.status === "Pending Verification");

    const normalizedSearch = deferredTableSearch.trim().toLowerCase();
    if (!normalizedSearch) return tabRows;

    return tabRows.filter((laptop) =>
      [laptop.barcode, laptop.brand, laptop.model, laptop.status].join(" ").toLowerCase().includes(normalizedSearch),
    );
  }, [deferredTableSearch, lotLaptops, tab]);
    ...
    <div data-page="receiving-verification" data-testid="page-receiving-verification" className="space-y-6 max-w-6xl">
      <div className="flex flex-wrap justify-between items-end gap-4">
        <div>
          ...
          <p className="text-sm text-cyan-500/40" style={{ fontFamily: "Share Tech Mono" }}>
            Scan-first verification • Progress tracking • Discrepancy notes
          </p>
        </div>
        <button
          data-testid="verification-complete"
          data-action="verification-complete"
          className="btn-cyber-green px-4 py-2 rounded-lg"
          disabled={total === 0}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card corner-marks p-5">
          <label
            className="block text-[10px] uppercase tracking-[0.12em] text-cyan-500/40 mb-2"
        ...
        <div className="glass-card corner-marks p-5">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span className="text-3xl font-black neon-text-cyan" style={{ fontFamily: "Orbitron" }}>
              {verifyPct}%
            </span>
            <div className="flex-1">
              <div className="progress-cyber h-3 mb-1">
                <div className="progress-cyber-fill" style={{ width: `${verifyPct}%` }} />
              </div>
              <p className="text-[10px] text-cyan-500/25" style={{ fontFamily: "Share Tech Mono" }}>
                {verifiedCount} of {total} verified ({verifyPct}%)
              </p>
              <p className="text-[10px] text-purple-300/40" style={{ fontFamily: "Share Tech Mono" }}>
                Graded: {gradedCount}/{total} ({gradePct}%)
              </p>
            </div>
          </div>
        </div>
        <div className="glass-card corner-marks p-5">
          ...
          <div className="flex gap-2">
            ...
            />
            <button className="btn-cyber" onClick={handleVerify}>
              VERIFY
            </button>
          </div>
        </div>
      ...
      <div className="glass-card corner-marks p-0 overflow-hidden">
        <div className="flex items-center gap-0 border-b border-cyan-500/10">
          {(["all", "pending", "verified"] as const).map((t) => (
            <button
              key={t}
              className={`px-5 py-3 text-[11px] font-bold uppercase tracking-[0.12em] transition-all border-b-2 ${tab === t ? "text-cyan-300 border-cyan-400 bg-cyan-500/5" : "text-cyan-500/25 border-transparent"}`}
              onClick={() => setTab(t)}
              style={{ fontFamily: "Orbitron" }}
            >
              {t} ({t === "all" ? total : t === "verified" ? verifiedCount : pendingCount})
            </button>
          ))}
        </div>
        <div className="px-4 py-3 border-b border-cyan-500/10 flex flex-wrap items-center gap-2 justify-between">
          <div className="flex flex-wrap items-center gap-2">
            <input
              className="w-full md:w-80 px-3 py-2 rounded-lg text-sm"
              placeholder="Filter by barcode/brand/model/status"
              value={tableSearch}
              onChange={(event) => setTableSearch(event.target.value)}
            />
            <button
              className="btn-ghost text-xs disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={tableSearch.trim().length === 0}
              onClick={() => setTableSearch("")}
            >
              Clear Search
            </button>
          </div>
          <p className="text-[11px] text-cyan-500/35" style={{ fontFamily: "var(--font-mono)" }}>
            Showing {filteredRows.length} of {tab === "all" ? lotLaptops.length : tab === "verified" ? verifiedCount : pendingCount}
          </p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredRows.map((l) => (
                <tr key={l.id}>
                  <td className="py-3 px-4 neon-text-cyan" style={{ fontFamily: "Share Tech Mono", fontSize: "11px" }}>
                    {l.barcode}
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <span
                      className={`cyber-chip ${l.status === "Pending Verification" ? "cyber-badge-yellow" : "cyber-badge-green"}`}
                    >
                      {l.status}
                    </span>
                  </td>
              ...
              {filteredRows.length === 0 && (
                <tr>
                  ...
                  >
                    No laptops match the selected tab/search filters. Try clearing search.
                  </td>
                </tr>
              )}
            </tbody>
